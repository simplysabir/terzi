...
impl Storage {
    ...
    async fn load(&mut self) -> Result<()> {
        ...
        if data_file.exists() {
            ...
            if !contents.is_empty() {
                self.data =
                    serde_json::from_str(&contents).unwrap_or_else(|_| StorageData::default());
            }
        }
    ...
    pub async fn list_collections(&self) -> Result<Vec<RequestCollection>> {
        let mut collections: Vec<RequestCollection> =
            self.data.collections.values().cloned().collect();
    ...
    pub async fn add_error_to_history(
        &mut self,
        request: &SavedRequest,
        error: &str,
    ) -> Result<()> {
    ...
    pub async fn get_history_stats(&self) -> Result<HistoryStats> {
        ...
        for entry in &self.data.history {
            ...
            if let Some(duration) = entry.duration_ms {
                ...
                stats.min_duration_ms = stats
                    .min_duration_ms
                    .map_or(Some(duration), |min| Some(min.min(duration)));
                stats.max_duration_ms = stats
                    .max_duration_ms
                    .map_or(Some(duration), |max| Some(max.max(duration)));
            }
        }
